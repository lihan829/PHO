...
\cdots\}\). For concreteness, assume a data set \(D\) equal to
...
positive data.  In the same fashion, \textsc{Contain} function
...
or others) and (3) generates the smallest set of structures consistent
...
address its two core functions, \textsc{NextSupFact} and 
...
limit the numbers of tones, moras, and syllables of an AR considered by the algorithm. 
Additionally, the algorithm initialises two empty sets: \( G \), representing the 
...
V \) which stores checked ARs to prevent the learner from revisiting
superfactors of those that have already been examined.

A queue \(Q\) is used to store structures awaiting to be checked by
the algorithm, following a standard breath-first search method. As shown in Line~\ref{dequeue} and \ref{enqueue},
...
into \(Q\), making it available for the learner to evaluate later in a breath-first
manner.
...
mora, and syllable of AR do not exceed \(t, m, s\),
...
determined to be missing from the dataset---that is, it is not a
...
melody tier or line crossing, will be excluded. Some linguistically
invalid expansion examples are shown in Figure~\ref{fig.invalid.AR}
...
\begin{figure}[ht]
	...
	\begin{minipage}[t]{0.2\textwidth}
		...
		\begin{tikzpicture}
			...
			\node (t2) at (1,2) {$\mu_2$};
			\draw [-] (t1) to (m1) to (s1);
			\draw[-, dotted] (t1) -- (t2); %node[midway] {$\times$};
		\end{tikzpicture}\label{ar:sup1x}
	\end{minipage}	\hfill
	% --- Third Subfigure ---
	\begin{minipage}[t]{0.2\textwidth}
		\centering
		...
		\end{tikzpicture}\label{ar:sup2x}
	\end{minipage}	\hfill
	\begin{minipage}[t]{0.2\textwidth}
		\centering
		\begin{tikzpicture} 
		...
		\end{tikzpicture}\label{ar:sup3x}
	\end{minipage}	\hfill
	\begin{minipage}[t]{0.2\textwidth}
		\centering
		\begin{tikzpicture} 
			...
			\draw [-] (s1) to (m2);
			\draw [-] (s1) to (m1);
		\end{tikzpicture}\label{ar:sup4x}
	\end{minipage}	\hfill
	
	\caption{Invalid expansions}
...
\begin{figure}[ht]
	...
	\begin{minipage}[t]{0.2\textwidth}
		...
		\begin{tikzpicture} 
			\node (s1) at (0,0) {$\sigma_1$};
			\node (s2) at (1,0) {$\sigma_2$};
			\node (m1) at (0,1) {$\mu_1$};
			\node (t1) at (0,2) {H};
			\node (m2) at (1,1) {$\mu_2$};
			\draw [-] (t1) to (m1) to (s1);
			\draw [->,dotted,thick] (s1) to (s2);
			\draw [-] (m2) to (s2);
			\draw [->,dotted,thick] (m1) to (m2);
		\end{tikzpicture}
		\vspace{1em}
...
universal rule has been debated \citep{goldsmith1976autosegmental,
	meyers1997ocp}. Our case study in Hausa enforces the OCP by only 
appending a tone different from the last tone in the current AR. There 
are two reasons for this practice. First, there is no clear evidence 
...
From a computational perspective, unrestricted growth of association lines is costly, as it allows arbitrary connections between any two nodes. % Even in a simple undirected graph with \(n\) nodes, there could be
% \(\frac{n(n-1)}{2}\) possible edges, which results a large search space. 
...
\begin{definition}
	...
	\textit{An edge $(t,m)$ in the tone--mora matrix is defined as the 
		\emph{final} tone--mora association if $t \in H \cup L$, $m \in 
		\mu$, and $(t,m) \in \alpha$, and there exists no edge $(t',m') \in
		\alpha$ such that $t' > t$ and $m' > m$.}
\end{definition} 

Once the final tone-mora association is identified at $(t, m)$, there are 
three options to form an association line $(t', m')$ while adhering to ER4:
...
\begin{figure}
	\centering
\includegraphics[scale=0.8]{fig.space.pdf}
\label{fig.AR.space}
...
line-crossing violation \kuo{bad.assoc1}. A column or
...
(as in \kuo{bad.assoc2} and \kuo{bad.assoc3}) as they violate the
WFC 1 and 2.
...
unassociated units as floating. We also leave this matter for future research.
...
corresponding heavy contour. This is, however, not accurate. As shown
in Table~\ref{sec3.tab.2typescontours}, Hausa accepts heavy HL but not
light HL, whereas Mende accepts heavy LH but not light LH.

In other words, the algorithm must be able to determine that none 
of the structures in Table~\ref{sec3.tab.2typescontours} contains 
...
that functions as a restriction discussed above, such that all
relations within $A$ are relatively preserved in correspondence with
those in $B'$. Definitions~\ref{def.submatrix} and~\ref{def.ar.matching} 
are adaptations of Definitions~\ref{def.res} and~\ref{def.contain}. 
\begin{definition}
	\label{def.submatrix}
...
Definition \ref{def.submatrix} defines what a submatrix is, which is analogous
to the general restriction definition in Definition~\ref{def.res}. Definition
\ref{def.ar.matching}
...
is provided in the Appendix, along with a detailed walk-through. 
...
whereas (\subref{fig:matrix-heavy}) is contained in (\subref{fig:matrix-super}) since the 
submatrix in (\subref{fig:matrix-super}) (the shaded cells) matches matrices of 
(\subref{fig:matrix-heavy}), in terms of associations
as well as tonal strings, but does not match those of (\subref{fig:matrix-light}).


\input{fig.matrixmatching.tex}


\subsection{Summary}
This section established BUFIA-AR, which is grounded in autosegmental 
theory. In particular, we explained the reasoning and implementation 
...
\section{Case Study: Hausa} \label{sec:hausa}
...
examples of which are shown in Table~\ref{sec4.hausa.ex}. As mentioned
...
\hijeff{the special letters won't show}
\begin{table}[h!]
	\centering
...
% \begin{tikzpicture}
	%     \node (bt1) at (-.4,0.8) {H};
	%     \node (bt2) at (0.4,0.8) {L};
	%     \node (bc1) at (-.4,0) {$\mu$};
	%     \node (bc2) at (0,0) {$\mu$};
	%     \node (bc3) at (0.4,0) {$\mu$};
	%     \node (bs1) at (-0.4,-0.8) {$\sigma$};
	%     \node (bs2) at (0,-0.8) {$\sigma$};
	%     \node (bs3) at (0.4,-0.8) {$\sigma$};
	%     \draw (bt1) -- (bc1) -- (bs1);
	%     \draw (bt2) -- (bc2) -- (bs2);
	%     \draw (bt2) -- (bc3) -- (bs3);
	% \end{tikzpicture}
...
containing glides (ƙw, ƙy, kw, ky, gw, gy) 
\hijeff{special letters won't show}
...
Table~\ref{sec4.tab.conversion} shows how an orthographic form with
...
\hijeff{I want to remove the matrix-coding from the table}
\begin{table}
		\centering
...
\toprule
		\textbf{Form} & Tones & \thead{AR Diagram}  & \thead{Matrix-based Coding} \\
		\hline
		\makecell{\textit{k\`oo.g\'ii} \\  `river'}		& LH	&
		\vspace{1em}
		\begin{tikzpicture}
		...
		\makecell[c]{	
			...
			\end{array}$} \\
		\hline
		\makecell{ \textit{mù.tûm} \\ `person'}	 & LF &
		\vspace{1em}
		\begin{tikzpicture}
			[scale=0.9, every node/.style={scale=0.9}]
			\node (t0) at (1,1) {L};
		...
		\hline
	\end{tabular}
	\caption{Corresponding Strings, AR Diagram, and Coding}
...
In this section, we provide two types of evaluation: 
qualitative and quantitative. The qualitative evaluation examines how the grammar learned 
...
\begin{table}[h!]
	...
	\begin{tabular}{p{5cm}p{5cm}p{5cm}}
		\toprule
		\textbf{Constraints Present in \newline both BUFIA and Baseline} 
...
constraint \kuo{cons.d}, whereas two constraints, \kuo{cons.a} and
\kuo{cons.b}, are needed to account for \textsc{Contour}{$_\mu$} is
...
\ea Hausa words exhibiting HL.H pattern (\textsc{*NonFinal-Fall})
...
\ea Hausa words exhibiting the L.L.H pattern
\label{llhwords}

\setlength{\tabcolsep}{0pt} % reduce horizontal padding
\renewcommand{\arraystretch}{1} % reduce vertical spacing
\begin{tabular}{@{}l@{\hspace{0.8em}}l@{\hspace{1em}}l@{}}
...
%Although cross-validation can in principle involve both
%positive and negative data, we do not generate negative ARs for
%evaluation. We only evaluate on held-out positive forms. This is
%because it is not straightforward to determine what counts as a
%meaningful “ill-formed” form in Hausa. Structurally ill-formed ARs
%(e.g., those containing crossing lines or mismatched tiers) are never
%generated by BUFIA-AR’s \textsc{NextSupFact} function. The
%construction of forms which are illicit according to the baseline 
%grammar would not make for a fair comparison
%against this baseline. Additionally, the only forms where BUFIA 
%constraints and baseline constraint disagree, as the qualitative
%analysis makes clear, involve forms which violate
%\textsc{*NonFinal-Fall} and \textsc{*Lapse}, some of which were
%actually present in the Hausa vocabulary. Finally, constructing
%grammatical and ungrammatical tonal forms would likely require a
%carefully designed native-speaker judgment study, which we leave 
%for future work. For these reasons, we only report accuracy over 
%positive held-out forms.
...
to a core idea behind Optimality Theory, which is that \textsc{Con} is a
...
as far as it goes, but we are aware of no theory of \textsc{Con} which provides
...
more complex than the ones attributed to \textsc{Con} to be included in a
grammar.
...
% BUFIA-AR and the grammars it outputs differ in important respects from
% other approaches. For one, BUFIA-AR is a categorical learner without
% probabilistic induction. As discussed in Section~\ref{sec:discussion.grammar}, however, we do
% not view this as a deficiency but as a deliberate design choice. Also,
% the grammars BUFIA-AR produces are sets of constraints, all of which
% must be satisfied, which stand in contrast to optimisation-based
% grammars.
...
BUFIA-AR. %It is also important to consider variants of BUFIA-AR which
% address languages with floating tones, OCP violations, and/or richer
% tonal inventories. Finally, BUFIA-AR should also be compared with
% alternative approaches to tonotactic learning.
...
structure and segmental features \citep{yip2002tone}. %For example, in Shanghai Chinese, the lower register tone only associates with syllables that have voiced obstruent onsets, but not voiceless ones \citep[also known as ``depressor consonants" in some African languages \citep{cassimjee1992tonology}]{chen2015shanghai}. 
...
The authors declare no conflicts of interest regarding the publication of
this paper.
...
\hijeff{do you have funding to report}
\paragraph{Data availability statement}
Replication data and code can be found in GitHub repository: \url{https://github.com/lihan829/BUFIA_AR.git}.

\paragraph{Ethical standards}
The research meets all ethical guidelines, including adherence to the legal requirements of the study country.

\paragraph{Author contributions}
HL curated data, conducted the case study, and wrote the first draft of the 
manuscript. HL and JH revised the manuscript and approved the final version.

\paragraph{Supplementary material}
State whether any supplementary material intended for publication has been provided with the submission.
\hijeff{So we have an apeendix - does it count as supplementry material?}
